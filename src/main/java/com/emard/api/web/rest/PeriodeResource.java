...
/**
 * REST controller for managing {@link com.emard.api.domain.Periode}.
 */
@RestController
@RequestMapping("/api")
public class PeriodeResource {
    ...
    public ResponseEntity<Periode> updatePeriode(@Valid @RequestBody Periode periode) throws URISyntaxException {
        log.debug("REST request to update Periode : {}", periode);
        if (periode.getId() == null) {
    ...
    public ResponseEntity<List<Periode>> getAllPeriodes() {
        ...
        List<Periode> page = periodeService.findAll();
    ...
    public ResponseEntity<Periode> getPeriode(@PathVariable Long id) {
