...
export class SecteurActiviteComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.secteurActiviteService
      ...
      .subscribe(
        (res: HttpResponse<ISecteurActivite[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInSecteurActivites(): void {
    this.eventSubscriber = this.eventManager.subscribe('secteurActiviteListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: ISecteurActivite[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/secteur-activite'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
